...
- Panel mounting and URL behaviour are chainable on `TB.tabs()`: `.lazy()` (mount only the active panel, horizontal layout), `.keepAlive()` (keep every panel mounted, preserving per-tab state), `.history('push' | 'replace')` (default `replace`), `.urlMode('server' | 'client')` (`client` rewrites the URL with no Inertia visit), `.syncUrl(false)` (drop URL sync entirely).
- `SkTabs` supports `v-model` on the active tab key and emits `change` with `{ key, previousKey, tab }` on every switch after mount. A URL deep link wins over a differing incoming `modelValue` on mount. The `empty` slot renders alone — no sidebar, no tab strip — when no tab is selectable.
- **Tab keys must be unique and non-blank.** `TabsBuilder.build()` throws on a duplicate key in dev and `console.error`s in production; `TB.item()` rejects a whitespace-only key; `.queryParam()` rejects an empty name the same way. `build()` returns an immutable snapshot, so a later `.addTabs()` cannot mutate an already-built config.
- `useUrlTab(tabs, paramName?, { history })` accepts a `ref` or a getter for `tabs` (read through `toValue()` on every access), tracks live `visible`/`disabled` state, and never resolves a disabled tab — neither from `?tab=` nor as the param-less default.
- `SkForm` exposes `reload()` for an on-demand `dataUrl` re-fetch; `.reloadOnDataUrlChange(true)` opts into refetching whenever the `dataUrl` prop changes after mount (default stays mount-only). In external `v-model` mode `initialData()`, a field's `.default()` and `dataUrl` seed the internal form only — they never populate the bound object.
- `FB.fileUpload().deferExistingRemoval(true)` turns removing an already-saved file from an immediate `DELETE /media/{id}` into a deferred one applied on save via `HasMediaCollections::syncMediaCollection()`. The drop zone accepts dragged files through the same `accept`/`maxFileSize`/`fileLimit` validation as the picker.
- **Form/field `disabled` is a floor, not a default.** `.props({ disabled: false })` cannot unlock a form made read-only by `.permission()`; `.props({ disabled: true })` still disables an otherwise-enabled field.
- Cross-page bulk selection **fails closed**: a `filter[...]` key the backend cannot apply 422s (`sk-bulk.unknown_filters`) instead of being dropped, and a `?columns=` value matching no declared column reduces every row to the `alwaysInclude()` keys. Send a selected row's id verbatim — `useDatatableSelection().executeBulkAction()` no longer coerces numeric-looking ids, and `ids` may be omitted entirely when `select_all_filtered` is `true`.
