...
- **`sk:install` is no longer documented as a recovery path for an existing project.** `docs/install.md` and `docs/update.md` described re-running it as an idempotent whole-project repair. It is not: every published path except `lang/` is overwritten without `--force`, the hash registry only protects a file you deleted rather than one you edited, and losing the git-ignored `storage/starter-kit/hashes.json` makes the command treat an installed app as a first install and copy `.env.example` over the existing `.env`. Both guides now carry the warning, and `docs/UPGRADE.md` records the boundary.
