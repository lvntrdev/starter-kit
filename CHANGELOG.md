...
- **Breaking: the kit's API documentation moved from Scramble's default `/docs/api` to the new api-dock panel at `/api-dock`.** Scramble's own routes are explicitly disabled from `register()`, so `/docs/api` and `/docs/api.json` now 404 on any app that updates — there is no dual-serving period. Anything bookmarking or linking the old URL must move to `/api-dock`, and a consumer who published their own `config/scramble.php` should check it no longer registers a competing docs route. See [docs/UPGRADE.md](docs/UPGRADE.md).
