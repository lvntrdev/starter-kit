...
| Missing Kit Dependencies | `missing-kit-dependencies` |
...
`MissingKitDependenciesCheck`, kit'in kendi `require` bloğunun listelediği bir Composer paketi tüketici uygulamada kurulu değilse FAIL döndürür (`composer.lock` daha yeni bir kit sürümünün gerisinde kalmıştır, ya da bir paket elle kaldırılmıştır). Bu kontrol olmadan, pakete bağımlı kit kodu çalışma zamanında belirsiz bir "class not found" hatasıyla çöker; kaynağı gösteren net bir teşhis alınamaz. Çözüm, raporlanan `composer update lvntr/laravel-starter-kit -W` komutunu çalıştırmak, ya da `sk:update`'in bunu sizin için çalıştırmayı teklif etmesine izin vermektir (aşağıya bakın). Eksik paket yoksa OK döner.
...
- `--modules=` `composer require` ile eklenip devreye alınacak opsiyonel gözlemlenebilirlik recipe'lerini seçer (`telescope`, `pulse`, `horizon`, `sentry`; virgülle ayrılmış ya da tekrarlanabilir). Boş bırakılırsa TTY'de interaktif olarak sorulur, `--no-interaction` ya da interaktif olmayan bir oturumda tamamen atlanır. Tanınmayan bir anahtar, hiçbir şey yazılmadan önce hızlıca hata ile durur. Tam recipe tablosu ve davranış için install.tr.md içindeki [Opsiyonel Gözlemlenebilirlik Recipe'leri](./install.tr.md#opsiyonel-gözlemlenebilirlik-recipeleri) bölümüne bakın.
...
**Kit'in gerektirdiği ama kurulu olmayan Composer paketlerini de raporlar.** Kit'in `require` bloğu, `composer.lock`'un gerisinde kalmış bir paket listeliyorsa, `sk:update` bunu yazdırır (`sk:doctor`'ın `missing-kit-dependencies` kontrolünün işaretlediği liste ile aynıdır) ve etkileşimli bir TTY'de, sizin için `composer update lvntr/laravel-starter-kit -W` çalıştırmayı teklif eder — reddetmek ya da prompt'un varsayılanını kabul etmek yalnızca komutu yazdırır. `--dry-run`, `--no-interaction` ve etkileşimsiz her oturum (CI, pipe'lanmış stdin) prompt'u her zaman atlar ve sadece komutu yazdırır. Buradaki başarısız bir `composer update` raporlanır ama `sk:update`'i başarısız kılmaz.
