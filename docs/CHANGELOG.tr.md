...
- **`sk:install` artık mevcut bir proje için kurtarma yolu olarak belgelenmiyor.** `docs/install.tr.md` ve `docs/update.tr.md` komutu idempotent bir proje-geneli onarım gibi tarif ediyordu. Değil: `lang/` dışındaki her yayınlanan yol `--force` olmadan da üzerine yazılır, hash kaydı düzenlediğiniz değil sildiğiniz dosyayı korur ve git tarafından yok sayılan `storage/starter-kit/hashes.json` kaybolduğunda komut kurulu uygulamayı ilk kurulum sayıp mevcut `.env` üzerine `.env.example` kopyalar. Her iki rehber de artık uyarıyı taşıyor, sınır `docs/UPGRADE.tr.md` içinde kayıtlı.
