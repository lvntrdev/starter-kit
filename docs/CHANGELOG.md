...
- **Static analysis (Larastan/PHPStan, level 5) now runs against `src/` in CI**, alongside Pint. `phpstan.neon` scans `stubs/app` so the package's `App\*` references (the consumer-owned classes the kit assumes exist, e.g. `App\Models\User`) resolve without a full consumer app; `phpstan-baseline.neon` snapshots the pre-existing findings so the gate holds only new code to the bar. Run locally with `composer analyse`.
